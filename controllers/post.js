...
exports.load = async (req, res, next, postId) => {
    try {
        ...
        if (post) {req.load = {...req.load, post};
            next();
        } else {throw createError(404, 'There is no post with id=' + postId);}
...
exports.create = async (req, res, next) => {
    ...
    let post;
    try {post = Post.build({title,body});
        ...
        try {
            ...
            console.log('----- Error: Error durante la creación del attachment -----');
            console.log(error.message)
        } 
    } catch (error) {
        if (error instanceof (Sequelize.ValidationError)) {
            console.log('----- Error: Error de validación -----');
...
exports.update = async (req, res, next) => {
    const {post} = req.load;
    console.log('Actualizando post -> ', post.id)
    ...
    try {
        ...
        try {
            ...
            console.log('----- Error: Error durante la actualización del attachment -----')
            console.log(error.message);
        } finally {
            res.redirect('/posts/' + post.id);
        }
    } catch(error){
        console.log('----- Error: Error durante la actualización del post -----')
        console.log(error.message);
        next(error);
    } 
}
...
exports.destroy = async (req, res, next) => {
    ...
    try {
        ...
        console.log('----- Error: Error durante el borrado del post -----')
        console.log(error.message);
        next(error);
    }
};